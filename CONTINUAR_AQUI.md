...
> Para retomar en un chat nuevo de Claude Code. **Estado a 2026-06-29, versión v0.17 (DESPLEGADA).**

## 🧵 HILOS ABIERTOS — EMPIEZA POR AQUÍ (2026-06-29 noche, tras v0.17)
...
> **A. 🌱 PROPONER RAMA → HACERLO VISUAL/CLICK, no por texto (Tony, 29-jun NUEVO, PRIORIDAD):** Tony, viendo el v0.17 funcionando, pidió que proponer una rama/sub-rama sea **clicando DÓNDE** quieres que salga (en qué rama/sitio del mapa), **NO rellenando un formulario**. Diseño objetivo: entrar en **«modo proponer»** → el usuario **TOCA en el mapa** el nodo/punto de donde sale la nueva (sub)rama → aparece una **«rama fantasma»** punteada ahí (geometría, sin texto de usuario en el SVG = anti-XSS) → luego le pone NOMBRE (el nombre sí es texto, inline). La **posición/padre se elige por CLIC**, no escribiendo. **= exactamente la propuesta «rama fantasma» (enfoque 2) que el panel de diseño había descartado por minimalismo en v0.17; ahora ES lo que Tony quiere.** El formulario actual (v0.17, `#mbPropose`, `window.__openPropose`) FUNCIONA y se queda como base/fallback; esto es el paso 2 inmersivo encima. Honestidad §9 + manda igual por `window.__fb` tipo `propuesta-rama` (el contrato con el loop NO cambia).
>   - ⚠️ **VERIFICAR EN NAVEGADOR primero:** Tony insinuó un problema al abrir nodos («node ya abrirlo aunque lo pulse pero hay un problema» — ambiguo). Sus capturas muestran el mapa Y el panel de proponer ABIERTOS (funcionan), y «Has abierto 1/8 ramas» (solo AHORA auto-abierta). Confirmar que **al TOCAR una rama se abren sus sub-ramas + entra el zoom** (puede ser solo descubrimiento, o un bug de interacción). Probar por DOM como en las tandas previas.
