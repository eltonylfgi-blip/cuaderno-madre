...
> Para retomar en un chat nuevo de Claude Code. **Estado a 2026-06-29, versión v0.14.**

## 🧵 HILOS ABIERTOS — EMPIEZA POR AQUÍ (2026-06-29 noche, para el chat nuevo)
> **🌳🌳 PRIORIDAD ABSOLUTA «PARA SIEMPRE» (Tony, 29-jun, literal): EL MAPA DE LAS RAMAS.** «Me tiene enganchado lo del mapa con las ramas porque puedo **entender bien MADRE desde ahí**; eso lo tenemos que poner como **prioridad absoluta para siempre**.» → El 🌳 mapa es **LA forma principal de entender y seguir MADRE**, no una tarjeta más. Todo lo demás va detrás. **Detalle COMPLETO del plan en `DESARROLLO_CUADERNO.md` §«🌳 v0.15+ — EL MAPA ES EL NÚCLEO» (léela ENTERA: visión del mapa + 2ª crítica ChatGPT + bugs).**
>
> **✅ v0.14 (DESPLEGADO, web viva sirve `v0.14`):** (1) 🐞 bug del bottom del mapa a pantalla completa ARREGLADO; (2) 1ª pantalla re-secuenciada (qué-es/por-qué primero, mapa bajado = recompensa, gancho 1ª persona); (3) «respira» nº1 = DIARIO VIVO (`#diarioVivo`: commits reales + «desde tu última visita: N»). Revisado por workflow adversarial 5 lentes. (Detalle abajo, secciones v0.14.)
>
> **⬜ LO QUE SIGUE (orden para el chat nuevo):**
> **A. 🐞 BUGS / pulido que Tony vio en v0.14 (rápido, ANTES de añadir):**
>   - **El HÁMSTER SIGUE SIN MOVERSE** (recurrente; `#envivo .wheel-and-hamster`). Verificar en navegador por qué la rueda no gira (¿restyle de `#envivo` pisa la animación? ¿`prefers-reduced-motion`? ¿el `font-size` de escala rompió los `em`?).
>   - **`#envivo`+`#diarioVivo` «se ve un poco mal hecho»** (apretado/tosco) y **«cosas demasiado pegadas al margen»** (paddings laterales, sobre todo móvil). Pulir aire/jerarquía; que el texto «AHORA» no choque con el hámster.
>   - **«🙌 ¿Quieres ayudar a MADRE?»**: dejar CLARO que **MADRE TAMBIÉN busca esas soluciones sola** (Tony lo preguntó), no solo pide ayuda al visitante.
> **B. 🌳 EL MAPA AL SIGUIENTE NIVEL (prioridad absoluta):**
>   - ⚠️ **Tony cree que «no hay sub-ramas» — SÍ las hay (v0.13, al tocar una rama en el overlay) pero NO las ve → es DESCUBRIMIENTO.** 1º verificar en navegador que salen; luego hacerlas OBVIAS.
>   - **Al clicar una rama → ZOOM hacia ella** (la cámara entra) + **sub-ramas multinivel «que se vayan creando»** (varios niveles; auto-orden desde `MAPA_RAMAS.tsv` horneable por el loop, buzón ya dejado) + **«cosas interactivas» al entrar** + **explicar cómo AVANZA MADRE en cada rama** (progreso por nodo) + **«explicar»/«no entiendo» por nodo** (`__fb`). **Decisión de Tony abierta:** ¿mapa como página aparte o seguir overlay?
> **C. UNA mecánica de «¿qué hará que quiera volver mañana?»** (la ÚNICA pregunta, según ChatGPT): revelado progresivo (desbloquear contenido al avanzar) / recompensa-tras-explorar («has descubierto 3 cosas», «entiendes más que el 90%», «desbloqueaste una nota privada») / **varias HISTORIAS ABIERTAS en paralelo** (¿morirá esta hipótesis? ¿subirá a 4/10? ¿publicará sola? ¿1ª estrella? ¿qué experimento hace AHORA?) / MADRE habla más en 1ª persona (frases-diario espontáneas) / SHOW-no-tell (timeline `08:14 hipótesis → 09:03 regla eliminada`) / 2-3 momentos «hostia» (votar, gráfico que cambia, hipótesis que muere en directo). **NO añadir tarjetas porque sí.** Detalle en DESARROLLO §v0.15+.
