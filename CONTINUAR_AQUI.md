...
## ⚠️ ESTADO DE PUBLICACIÓN — LÉEME PRIMERO (2026-06-29, v0.13)
- **v0.10 = COMMITEADA EN LOCAL, SIN PUSH** (commits `091448c` + `9fa98fd`; `main ahead 2` de `origin`).
- **v0.11 + v0.12 + v0.13 = cambios GRANDES en el WORKING TREE, SIN COMMIT.** v0.11 = tanda larga de feedback. v0.12 = motor ASMR por textura + página de audición. **v0.13 = el 🌳 mapa a PANTALLA COMPLETA inmersivo (zoom/pan + sub-ramas + AHORA, deep-link `#mapa`) + «ventana viva» (`#envivo` abre con «📍 AHORA» + «🌙 Mientras no mirabas» + frase citable)** (esta sesión). **Verificado en navegador: 0 errores/0 warnings; `node --check` de los 26 scripts OK; revisado por workflow adversarial (12 hallazgos corregidos); móvil 375px OK; deep-link no saca de la página.** Pendiente: que Tony decida **commit + push de v0.10 + v0.11 + v0.12 + v0.13 juntos**.
- **🔴 La rutina `cuaderno-feedback` SIGUE PAUSADA a propósito** (`enabled:false`). Se pausó para (a) no colisionar con la edición del `index.html` y (b) NO auto-pushear durante el gate de Tony. **NO la reactives hasta que Tony decida el push:** al reactivarla, su próxima pasada haría `git add` + commit + `git push`, arrastrando v0.10 **y** v0.11 **y** v0.12 **y** v0.13 a internet. Reactivar cuando toque: tool `update_scheduled_task` → `cuaderno-feedback` `enabled:true` (o desde la app de Tareas).
