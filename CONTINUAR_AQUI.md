...
> ✅ **1. 🐞 BUG del bottom del mapa (`#mapBig-ov`) — ARREGLADO (v0.14).** Causa real (verificada en preview, no asumida): en viewports estrechos+BAJOS el `flex:1 1 auto` del `.mbStage` se aplastaba (mapa a 75px) y la columna de zoom (148px) se salía tapando la barra. Fix: bloque inferior en `.mbBottom{max-height:42vh;overflow-y:auto}` (acota+scrollea) + `@media(max-height:480px)` (barra compacta, zoom en FILA) + `@media(max-height:400px){.mbBottom:34vh}`. Probado a 240/300/430/812px alto y 667×360.
> ✅ **2. 1ª pantalla RE-SECUENCIADA (v0.14).** Orden nuevo: `onboard → vsCard (qué es 10s) → hero (3/10) → abiertasCard (por qué volver) → envivo → 📓 diario → … → camino → 🌳 mapa (bajado = recompensa)`. Gancho `#onboard` en 1ª persona + pregunta plantada + menos texto. *(La auditoría de funciones «si quito X ¿se pierde algo?» quedó ligera; profundizarla es opcional.)*
> ✅ **3. «RESPIRA» nº1 = DIARIO VIVO (v0.14).** `#diarioVivo` tras `#envivo`: commits REALES del repo (API GitHub) con hora + enlace, auto-refresh 3 min, «✨ nuevo», y «🌙 desde tu última visita: N cambios» (localStorage). Honesto §9, degrada en silencio. **FALTA el 2º mecanismo** («mundo→MADRE»: «alguien comentó (Argentina) → MADRE lo leyó» desde `comments` de Supabase; `#envivo` ya tiene «última señal vuestra», ampliar eso). Otros de la lista (pensando-ahora, cambios espontáneos sin recargar, «mientras leías cambié de opinión») por hacer.
> ⬜ **4. 🌳 Mapa siguiente nivel (PENDIENTE):** sub-ramas de TODO (varios niveles) + botón «explicación»/«no entiendo» por (sub)rama (reusar `__fb`) + auto-orden según lo que emerge (vía `MAPA_RAMAS.tsv` horneable, buzón ya dejado). **Decisión de Tony pendiente:** ¿mapa como página/acceso directo aparte o seguir overlay? · ⬜ **Secciones B–E** (historia/personaje, progreso con MOVIMIENTO, momentos wow, realidad cruda visible) por hacer.
> A Tony LE ENCANTA el mapa grande («que chuloo»). v0.14 hizo los pasos 1-3 (verificado en navegador 0 errores + revisado por workflow adversarial de 5 lentes; fixes reales aplicados). **Lo de arriba (4 + B–E) es lo que sigue.**
